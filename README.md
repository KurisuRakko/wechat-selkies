...
dashboard 每次加载都会强制写入以下设置，并把对应控件设为禁用，避免浏览器旧的 `localStorage` 值在重连时重新生效：
...
侧边栏的 **应用程序** 和 **共享** 两个整块面板会被隐藏。视频比特率滑块保持“越大越好”的自然方向；静态区域优化底层的 CRF 滑块使用 RTL 方向，画质最好的 `min` 在右侧、画质最差的 `max` 在左侧，滑块旁边的数字仍显示真实 CRF。界面缩放使用 Selkies 的 `scaling_dpi=168`（175%），服务端会自行通过 `xrdb` 应用 `Xft.dpi`，因此无需再改容器启动脚本。

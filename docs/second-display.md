...
- 默认在计数 0→>0 的边沿自动尝试一次 `window.open` 打开副屏窗口；因为轮询
  回调不是用户手势，多数浏览器的弹窗拦截器会拦下这次尝试，自动退化为页面
  顶部提示条；用户给站点放行弹窗后才会真正自动打开。同一批（计数保持 >0）
  只尝试一次，用户手动关掉副屏窗口后同一批不会重开。可以在侧边栏"副屏
  窗口"卡片里关闭该行为（见下文"客户端设置"）。
- 不会像对待非主窗口那样把微信主窗口平铺/级联到别处；登录二维码窗、强制
  登出后的提示弹窗同样不搬。这些窗口的识别规则和
  `wechat-window-watchdog.sh`（主窗口看门狗）完全一致，两者不会对"这是不是
  主窗口"产生分歧。唯一的例外：如果 openbox 自己把主窗口错误地拖到了副屏
  区域（见下面"已知限制"），窗口管家会把它抢回主屏——这是纠正 openbox 的
  副作用，不是"搬动"主窗口去平铺。
...
侧边栏"副屏窗口"卡片（视频设置卡下方）暴露两项客户端偏好，都存浏览器
localStorage，改动即时生效、无需刷新页面：

| 设置 | 默认值 | 说明 |
|------|--------|------|
| 副屏打开方式 | 弹出窗口 | 弹出窗口：`window.open` 带 features `popup=yes,width=1280,height=800`；标签页：不带 features，浏览器按普通标签页打开 |
| 检测到子窗口自动打开副屏 | 开 | 每次计数 0→>0 边沿自动尝试一次打开副屏窗口（被弹窗拦截时退化为提示条，见上文"不做的事"）；关闭后只显示提示条，不再自动尝试 |

两项分别存 localStorage 键 `wechatSecondDisplayPopupMode` /
`wechatSecondDisplayAutoOpen`，清浏览器缓存或换浏览器后恢复默认值。
...
- **副屏连接/断开/改分辨率会让主屏视频短暂重置一次**——这是 Selkies 上游
  ...
  预期会看到主屏画面短暂闪一下。
- **副屏连接瞬间，openbox 可能短暂把主窗口拉过去**——这同样是上游重划
  RandR 布局时 openbox 自己对"屏幕变了"的反应，不是本项目代码引发的；
  窗口管家每轮 reconcile 都会检查主窗口是否仍在主屏上，不是就用
  `wmctrl`/`xdotool` 抢回来，因此这最多持续一个 reconcile 周期
  （≤3 秒，通常更快），不需要用户干预。

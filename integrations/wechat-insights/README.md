...
| 深度 Depth | TA 消息平均长度（0.12）、疑问句占比（0.09）、长消息 >50 字占比（0.09）；llm 策略下另加 LLM 时段深度分（0.35）与亲密度分（0.35）<br>（词法策略下：三项按 0.4/0.3/0.3 归一） |
| 恒常 Constancy | 有往来的天数占比（0.4）、当前已沉默的天数（0.35，越久越差）、窗口内最长断联（0.25，越长越差） |
| 对等 Reciprocity | 双方共同投入下限日均（0.35）、消息量均衡度（0.20）、字数均衡度（0.15）、LLM 时段双向性分（0.30）<br>（发起对话均衡度已删除：同一件事已由主动维计罚，不重复计罚） |
...
打分口径版本（维度组成、权重或 LLM 注入方式）变化时，下一轮分析会自动清掉回放标记
...
「当前沉默天数」按窗口内最后活跃日与窗口终点之差计算（打分时刻的消息时间戳只对当下
成立）。
...
默认关闭，开箱即用的是上面的纯词法深度代理。想用大模型评分时：
...
**画像调用**：给候选联系人（采样窗口内聊过天，且画像过期或新增消息达标）
采样最近 `INSIGHTS_LLM_SAMPLE_DAYS` 天（默认 60）的几段对话，生成三样东西：
两三句话的关系画像摘要（主要聊什么话题、什么相处模式）、2–4 个话题标签
（如 游戏、深夜谈心、工作吐槽，概括主要话题与相处方式，给不出就是空数组），
以及（当检测到近期异动时）一句话的异动原因推测。异动原因会连同当时的异动
清单指纹一起缓存，只有异动清单变了才重新推测——页面上的解释永远对应当前
展示的异动。三样产物缓存在 `metrics.db` 的 `llm_depth` 表里。标签与画像都在
详情页的「关系画像」卡展示，列表卡片名字下方再显示最多两个小标签；老缓存
行没有标签的（升级前评过的）会在下一轮分析自动重评补齐，多数部署两天内
收敛。

**时段调用**：给每个自然月（双方文字消息 ≥ `INSIGHTS_LLM_PERIOD_MIN_TEXTS`，
默认 30）的联系人评一次，产出 depth（话题实质与自我袒露）/ warmth（情感
亲密度）/ mutuality（双向性）三个 0–100 分，快照存进 `llm_period` 表。未收口
的当月每 `INSIGHTS_LLM_PERIOD_REFRESH_DAYS` 天（默认 7）重评一张新快照（旧
快照保留，供旧时刻的回放取用），月份收口后再补评一次覆盖整月。三个分按
`period_end ≤ 打分日` 可见性注入七维：depth+warmth 各 0.35 进深度维（词法
三项权重相应缩到 0.12/0.09/0.09），mutuality 0.30 进对等维；没评过分的
联系人缺值自动回流给其余项，不需要任何特判。出站样本从整个时段里均匀
抽取、过 `mask()` 后才发送，不携带任何姓名。

首次上线会做一次全量回填：约 1265 次调用（本仓库规模），60–90 分钟跑完。
两笔预算互不挤占：近期维护（当月刷新 + 刚收口月份补评）上限
`INSIGHTS_LLM_PERIOD_MAX_CALLS_PER_RUN`（默认 40），历史回填上限
...
- `INSIGHTS_LLM_REFRESH_DAYS`：画像保鲜期，默认 30 天。
- `INSIGHTS_LLM_REFRESH_MESSAGES`：画像后新增这么多条消息就重评，默认 200。
- `INSIGHTS_LLM_MAX_CALLS_PER_RUN`：单轮画像调用上限，默认 40——
...
- `INSIGHTS_LLM_TIMEOUT_SECONDS`：单次请求超时，默认 30 秒，失败自动重试
  一次（共至多两次请求）。
- `INSIGHTS_LLM_PERIOD_MIN_TEXTS`：时段评分的双方文字条数门槛，默认 30
  （5–1000）。
- `INSIGHTS_LLM_PERIOD_REFRESH_DAYS`：未收口当月的重评间隔，默认 7 天
  （1–90，调小换更细的当月快照，代价是调用数线性上升）。
- `INSIGHTS_LLM_PERIOD_MAX_CALLS_PER_RUN`：单轮近期时段调用上限，默认 40
  （0–2000）。
- `INSIGHTS_LLM_HISTORY_MAX_CALLS_PER_RUN`：单轮历史时段回填上限，默认 120
  （0–5000），与近期预算互不挤占。
...
| `INSIGHTS_DEPTH_STRATEGY` | `lexical` | 深度维度策略：`lexical`（词法代理）或 `llm`（大模型评分，见上） |
| `INSIGHTS_LLM_BASE_URL` | 空 | OpenAI 兼容端点；空 = 禁用全部 LLM 调用（评分/画像/分类） |
...
| `INSIGHTS_LLM_PERIOD_MIN_TEXTS` | `30` | 时段评分的最低双方文字条数（5–1000） |
| `INSIGHTS_LLM_PERIOD_REFRESH_DAYS` | `7` | 未收口当月的重评间隔（1–90 天） |
| `INSIGHTS_LLM_PERIOD_MAX_CALLS_PER_RUN` | `40` | 单轮近期时段调用上限（0–2000） |
| `INSIGHTS_LLM_HISTORY_MAX_CALLS_PER_RUN` | `120` | 单轮历史时段回填上限（0–5000）；首次上线可临时调大到 2000 |

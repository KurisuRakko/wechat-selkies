...
- 每天固定时刻（默认 04:30）跑一轮**增量**分析：按会话游标只读新消息，
  首轮做全量回填。可通过 `INSIGHTS_ANALYZE_TIME` 配置多个时刻（逗号分隔）。
- 消息原文只在内存里过一遍词法统计（字数 / 是否疑问句 / 「哈」连击），随后立即丢弃。
  `metrics.db` 里**只有统计数字**，没有任何聊天内容。
- 结果预计算进 `scores` 表，页面打开是秒开的，HTTP 处理器不做全量扫描。
- 右键联系人卡片（详情页在页头右键）可以标记「感觉偏高/偏低」：标记本身不立即改分，
  下一轮分析把幅度分配到七个维度（方向由标记决定），叠加进综合分与七维并带角标，
  可随时清除还原；温度历史曲线始终保持客观口径、不吃校准。
...
手动好感度校准同样只影响看板展示：温度曲线记录的一直是校准前的客观综合分，
「清除了校准」与「从来没有校准过」在曲线上看不出区别。
...
- **已经绝交（certain）**：用户确信已经绝交。核实优先级：AI 从标记日前后各 7 天
  ...
  合计 ≤ 5 条、且已过 14 天）→ 确认；AI 判「仍在正常聊天」→ 否决标记；证据不足时
  按用户断言直接生效——没有数据可以反驳，就相信用户。
- **我认为的绝交（suspected）**：存疑标记。核实流程同上，但证据不足时**保留标记
  下一轮再核**（沉默证据会随时间成熟），不会直接生效；AI 判破裂或冷断统计达标时
  照常确认。

确认后该联系人的综合分与七维被整体等比压缩，封顶为 **10 分（已经绝交）/ 20 分
（我认为的绝交）**，封顶作用在校准之后的最终值上。列表卡片与详情页页头会出现
红色「绝交 -52.3」角标（悬停显示判定类型与一句话依据）；核实否决的显示「绝交存疑」
角标、分数不动；标记排队中显示灰色「绝交核实中」。右键菜单里的「清除绝交标记」
即时还原分数与角标。绝交只压低看板展示分，温度历史曲线始终保持客观口径，与校准
一样「清除了绝交」和「从来没有绝交过」在曲线上看不出区别。确认绝交后，详情页的
关系温度曲线只显示到绝交那天（当天含），曲线数据本身不会被删除，清除标记后完整
曲线立即恢复。

吵架判定只在 `INSIGHTS_DEPTH_STRATEGY=llm` 下启用；冷断统计判定不依赖 LLM，任何
策略下都会跑。采样文本整体过 `mask()` 后才出站。
...
| 变量 | 默认值 | 说明 |
| --- | --- | --- |
| `INSIGHTS_ANALYZE_TIME` | `04:30` | 每天跑分析的时刻，`HH:MM`；逗号分隔可配多个（如 `04:30,12:30,20:30`），容器本地时区；非法项忽略，全部非法回退 04:30 |
...
| `INSIGHTS_CALIBRATE_MAX_PER_RUN` | `20` | 单轮好感度校准最多消化的标记数（1–500） |

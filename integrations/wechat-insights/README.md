...
**全史回放**：首次部署会顺带把今天之前的历史也回放一遍——从联系人最早有消息的那一天
起，按每周一点的网格逐日推进到今天，用同一套打分内核算出每一点的分数（7 天窗口、等效
天数、百分位、事务往来剔除全都一致）。因此曲线不是从部署日才起步：详情页的「关系温度」
卡一上来就有完整的多年起伏，每周一点 + 部署日起每日一点。回放是一次性的，跑完在 meta
表写 `score_history_backfilled` 标记，之后每轮分析只补当天的点；环境变量
...
日点混合时不会平均铺开失真）：部署当天还凑不出两个采样点时整卡不渲染。七维分也一并
入库备用，目前只下发综合分。

...
同一次调用还会生成两样东西：两三句话的关系画像摘要（主要聊什么话题、
什么相处模式），以及（当检测到近期异动时）一句话的异动原因推测。异动
原因会连同当时的异动清单指纹一起缓存，只有异动清单变了才重新推测——
页面上的解释永远对应当前展示的异动。两者都只在详情页展示，同样受出站
敏感词屏蔽约束。
...
| `GET /api/contacts` | 全部联系人的七维分、趋势、近 30 天消息数 |

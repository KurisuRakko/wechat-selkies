...
形成一条「每天一个点」的历史曲线，存进 `metrics.db` 的 `score_history` 表（同一天多次
分析只留最后一次的分数）。数据不足的联系人不记——没有分数就没有温度；归零照记
（0 分），曲线能看出「滑落」的过程而不是突然消失。

详情页的「关系温度」卡用折线展示这条曲线（综合分 0–100）：部署当天还凑不出两个采样
点时整卡不渲染，曲线只会从部署日起越攒越长。七维分也一并入库备用，目前只下发综合分。
...
意图是在归零之前抓住正在滑落的高分关系——看板从观赏变成行动。无命中时不显示，名单
每轮分析整体重算，不会残留旧条目。
